...
export class PlayerEditDialogComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.playerViewService.getPlayer(this.Id).subscribe(
      res => {
        this.player = res["player"] as Player;
        if (this.player.photo != null) {
//          this.player.photo = btoa(this.player.photo);
          this.player.photo = btoa(String.fromCharCode(...new Uint8Array(this.player.photo)));
        }
